...
.DESCRIPTION
    ...
    NovaClient-WinPS5.ps1 instead. The two scripts are deliberately identical
    below the HTTP TRANSPORT section so they can be diffed against each other;
    only that one region differs.
...
function Invoke-NovaMain {
    if ($ShowVersion) {
        Write-Host "nova-client (PowerShell 7) $Script:NovaClientVersion"
        return 0
    }
